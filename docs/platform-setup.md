...
> 注意：ClawHub slug、`skill.json` / SKILL.md frontmatter 中的 `name`、以及实际安装目录现已统一为 `insentek-openapi`。旧 slug `insentek-api-skill` 已被废弃，仅作历史保留。

#### 方式三：本地文件安装

```bash
# 从本地 SKILL.md 安装
openclaw skills add ./SKILL.md
...
或在 Web UI 中：Skills → 找到 "insentek-openapi" → **Remove**。
...
# 复制 skill 文件
Copy-Item skill.md "$env:USERPROFILE\.claude\skills\insentek-openapi.md"
```

**macOS:**
```bash
mkdir -p ~/.claude/skills
cp skill.md ~/.claude/skills/insentek-openapi.md
```
...
- **全局级（推荐）**：从 `~/.claude/skills/` 目录移除对应文件
  ```bash
  # Windows
  Remove-Item "$env:USERPROFILE\.claude\skills\insentek-openapi.md"

  # macOS / Linux
  rm ~/.claude/skills/insentek-openapi.md
  ```
...
# 复制 skill 文件
Copy-Item skill.md "$env:USERPROFILE\.hermes\skills\insentek-openapi.md"
```

**macOS:**
```bash
mkdir -p ~/.hermes/skills
cp skill.md ~/.hermes/skills/insentek-openapi.md
```
...
**Windows:**
```powershell
Remove-Item "$env:USERPROFILE\.hermes\skills\insentek-openapi.md"
```

**macOS / Linux:**
```bash
rm ~/.hermes/skills/insentek-openapi.md
hermes skill reload
```
