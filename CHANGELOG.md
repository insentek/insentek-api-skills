...
- **移除 README 中向 Agent 提供凭据的反例对话**
  - 此前 `README.md` 「开始对话」一节使用 `User: 我的 appid 是 xxx，secret 是 yyy，查看所有设备` 作为示例，与项目核心约束（Agent **禁止**在对话中接收 / 索要 `appid` / `secret`）相违背，可能误导 Agent 行为或用户习惯
  - 替换为合规的自然语言对话示例（`查看我的所有设备` / `1号大棚现在的土壤温度` / `导出 3 号设备最近 7 天数据为 CSV`），并在示例上方显式说明凭据已在 `login` 步骤本地加密保存，不要也不需要在对话里发送
