...
export async function uninstallSkill({ runtimeId, scope, context = {} }) {
...
export async function uninstallSkills({ runtimeIds, scope, context }) {
  const results = [];
  for (const runtimeId of runtimeIds) {
    results.push(await uninstallSkill({ runtimeId, scope, context }));
  }
  return results;
}
